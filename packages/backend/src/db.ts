import { MemberState, SessionSetting, defaultSessionSetting } from "@kikoune/shared";
...
export const setMemberState = async (roomId: string, userId: string, state: MemberState) => {
  await redis.set(roomUser(roomId, userId), JSON.stringify(state), "EX", 15);
};
const roomUser = (roomId: string, userId: string) => `room:${roomId}:user:${userId}`;
...
export const createSession = async (roomId: string, host: string): Promise<DbSession> => {
...
export const enqueueVideo = async (roomId: string, videoId: string, userId: string) => {
  const sessionRaw = await redis.get(roomSession(roomId));
  if (!sessionRaw) {
    throw new Error("Session does not exist");
  }
...
export const dequeueVideo = async (roomId: string, session: DbSession): Promise<void> => {
  let videoId: DbSessionVideo | undefined;

  if (session.setting.random) {
    videoId = session.queue.splice(Math.floor(Math.random() * session.queue.length), 1)[0];
  } else {
...
export const getOrCreateSession = async (roomId: string, host: string): Promise<DbSession> => {
