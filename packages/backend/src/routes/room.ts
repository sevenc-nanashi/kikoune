...
import { SessionSetting, buffer, maxMessageLength, sessionSettingSchema } from "@kikoune/shared";
...
app.put(
  ...
  async (c) => {
    return await lock.acquire(c.req.param("id"), async () => {
      ...
      const memberStates = await db.getMemberStates(c.req.param("id"), data.userIds);
      ...
      const session = await db.getOrCreateSession(c.req.param("id"), c.get("userId"));
      const video = session.video ? await getVideo(session.video.videoId) : null;
      if (!video || session.startedAt + video.length * 1000 + (buffer + 2000) < Date.now()) {
        if (!(!session.video && session.queue.length === 0)) {
          ...
          for (const userId of data.userIds) {
            ...
            if (memberStates[userId].message || memberStates[userId].rotate) {
              ...
              await db.setMemberState(c.req.param("id"), userId, memberStates[userId]);
            }
          }
        }
      }
      if (!memberStates[session.host]) {
        log.info(`[${c.req.param("id")}] Host is not in the room, moving to ${c.get("userId")}`);
...
app.post(
  ...
  async (c) => {
    ...
    const canQueue =
      ...
      (session.queue.length < session.setting.queueLimit && !session.setting.queueLocked);
    if (!canQueue) {
      c.status(403);
    ...
    await lock.acquire(c.req.param("id"), async () => {
      await db.enqueueVideo(c.req.param("id"), c.req.valid("json").videoId, c.get("userId"));
    });
...
app.post(
  ...
  async (c) => {
    return await lock.acquire(c.req.param("id"), async () => {
      ...
      if (session.video.requestedBy !== c.get("userId") && session.host !== c.get("userId")) {
        c.status(403);
...
app.delete("/:id{[0-9a-z-]+?}/queue/:nonce{[0-9a-z-]+?}", async (c) => {
  return await lock.acquire(c.req.param("id"), async () => {
    ...
    if (video?.requestedBy !== c.get("userId") && session.host !== c.get("userId")) {
      c.status(403);
      return c.json({ error: "Forbidden" });
    }
