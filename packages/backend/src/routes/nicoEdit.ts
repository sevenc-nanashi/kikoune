...
const replaceToExternal = (src: string) =>
  src
    ...
    .replace(/https?:\/\/assets\.embed\.res\.nimg\.jp\/js\/(.+?).js/g, "/nico/watch/$1")
...
app.get("/watch/:id{.+}", async (c) => {
  const id = c.req.param("id");
  const js = await fetch(`https://assets.embed.res.nimg.jp/js/${id}.js`).then((res) => res.text());
...
app.get("/users/:id{.+}", async (c) => {
  const id = c.req.param("id");
  const userData = await fetch(`https://embed.nicovideo.jp/users/${id}`).then((res) => res.text());
